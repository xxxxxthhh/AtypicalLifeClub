...
| PR-2 | `generate_briefing.py` + `validate_briefing.py` + 单测。**先不接 workflow**，用 `--date` + `--since-sha` 回放历史日期人工核对 | 2026-08-28 回放：**期望值待 PR-1b 落地后重新冻结**（污染基线下是「只出黄金」，干净基线下是「只出钯金」——绝不可在 PR-1b 之前把任何一个写成测试契约）；**2026-08-31 回放：研究段必须出 MiniMax H1（证据日 08-26）——这条是新旧选取键的分水岭，旧规则会丢掉它**；周末回放：`empty: true`；同日两次触发回放：第二次不得丢掉第一次的条目 |
