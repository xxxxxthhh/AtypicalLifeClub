...
| **污染范围只有这两只** | 同日对照：GLD `417.29`、SLV `69.04`、COPX `83.05` —— 仓库值与 Yahoo 当前返回值**完全一致**。5 个金属期货无拆分无分红 | 修正只需碰 PPLT/PALL 在 2026-05-18 之前的行。**但不要用全量重拉去做**，见 §3.1 |
...
| Fact | Value | Consequence |
|---|---|---|
| **金属必须用收益率 z，不能用价格 z** | 2026-08-28：黄金价格 z(1y) ≈ **+0.54σ**（毫无信号），收益率 z(90d) ≈ **-1.9σ**（正确读数）| 黄金一年里是趋势上涨的，价格 z 只能说"现在价格偏高"，回答不了"今天这根跌是不是异常"。简报问的是后者。**结论成立，但具体 σ 值以 PR-1b 后的干净基线为准**——原稿的 -2.22σ / +1.78σ 取自被污染的期货序列（见下行）|
| **🔴 期货未完成时段被当收盘写入（PR-1b）** | 5 个期货各约 111–114 行与 Yahoo chart API 不符，中位偏差 0.57%–1.12%、最大 7.12%，**全部始于 2026-02-08**（日更接管当天）。仓库另有 28–29 个 chart 侧不存在的日期，**其中 25 个是周日**。根因：期货周日晚开盘，`fetch_latest` 取 `df.iloc[-1]` 把未完成时段当日收盘写入且永不回补。与 `update-research-prices.yml` 注释里记录的亚洲盘中报价 bug 同类 | **足以翻转 2σ 判定**：2026-08-28 回放，污染数据下黄金 z=-2.22（触发）、钯金 z=+1.78（不触发）；chart 基线下黄金 z=-1.89（不触发）、钯金 z=+2.67（触发）——**结论完全相反**。PR-2 的回放验收必须等 PR-1b |
...
**🔴 不要跑 `fetch_historical.py` 全量重拉**（原稿如此建议，实施时否决）。它的 `LOOKBACK_DAYS = 730` 只回溯到约 2024-09，而现有数据自 **2024-02-08** 起——全量重拉会**静默截掉全部 14 个标的约 7 个月历史**。

改为**定向回改**：把 PPLT / PALL 在 2026-05-18 之前的 `close` 分别除以 10 / 5。逐行与 Yahoo 的复权序列比对后再写入（实施时 569/569 行零例外）。

验收：`PPLT` 2026-05-15 收盘由 `179.03` 变为 `17.903`；`PALL` 由 `128.77` 变为 `25.754`；GLD/SLV/COPX 及 5 个期货**逐行不变**；`current` 不变；PPLT 90 日波动率 9.79% → 2.47%。
...
- **B**：每周一次全量重拉覆盖（复用 `fetch_historical.py`），日更仍只 append。实现更简单，代价是每周一次大 diff，拆分最长可潜伏 6 天，**且必须先把 `LOOKBACK_DAYS` 提到覆盖现有全部历史，否则每周截一次历史**。

无论选哪个，`metadata` 都要留下可审计的痕迹，**不能静默改写历史**。机器写的调整记录进 `metadata.splitAdjustments[]`，与人写的 `knownSplits[]` 白名单**分开**——否则日更会自己往白名单里加条目，白名单就变成了消音开关。

**🔴 必须 fail-closed：任一标的的拆分查询失败即整次不写文件。** 「查不到」与「没有拆分」不是一回事。既然 §3.3 的阈值明确把小拆分委托给这一层，这一层失败还照常追加当日价格，未复权的拆分就能同时绕过两道防线。

### 3.3 补连续性校验（本次事故的真正漏网点）

`validate_data.py` 新增：任一标的单日 |涨跌| > **35%** 即 **fail**，除非该日期在 `metadata.knownSplits[]` 白名单里。白名单条目需含 `symbol` / `date` / `ratio` / 一句人写的理由（理由为空即 fail）。

**🔴 阈值必须按数据定，原稿的 25% 会让 CI 在真实行情上挂。** 9082 个连续对里最大的**真实**变动是 SI=F **-31.35%**（2026-01-30），由同日 SLV -28.54%（独立标的、无拆分）与 PL=F -19.04% 交叉印证为一次真实贵金属普跌；最小的正向拆分 2:1 ＝ -50%。35% 落在两者之间，当前文件 0 触发。**绝不能把 SI=F 那天写进 `knownSplits`**——它不是公司行为，`ratio` 对它没有意义，那就是 §9 说的消音开关。

已知缺口：3:2（-33%）这类小拆分低于阈值。这是**故意的**——第一道防线是 §3.2 的拆分源检测（与幅度无关），本条只是它静默失败时的网，所以只针对大到会毁掉波动率序列的那些。**不要为了接住小拆分而下调阈值、靠白名单补**。
...
| 金属 | `metals/data/historical.json` | **收益率 z**，完整口径见 §4.1.1；\|z\| ≥ **2.0** 才列 |
| 汇率 | `currency/data/historical.json` | **价格 z**：365 天窗口 + 样本标准差，与 `check_alerts.py` 一致。四个货币对**全部列出**（换汇是持续决策，不只在越线时才有意义），越 1.5σ / 2.0σ 的加标记 |

#### 4.1.1 🔴 金属收益率 z 的完整口径（不得再有解释空间）

原稿只写「90 日均值 / 90 日标准差」，实测下来**这句话的歧义足以改变简报内容**。

- **标的范围**：`metals` 与 `etfs` **全部 14 个标的**同等参与筛选（5 期货 + 9 ETF）。§3.1 把 PPLT 未复权列为阻塞项，理由正是它「在简报里静默失灵」——ETF 本来就在筛选范围内。
- **收益率**：`r_t = close_t / close_{t−1} − 1`（简单收益率，与 `changePct` 同口径）。`close_{t−1}` 取序列中前一个可用 bar。
- **窗口**：**当日之前的 90 个收益率**（不是 90 个日历日）。这需要 **92 根 close**——90 个基线收益 + 1 个当日收益 = 91 个收益 = 92 根 close。冻结 fixture 每个标的正好 92 根，就是这么来的。
- **窗口位置**：**不含当日**——基线是 `r[-91:-1]`，当日是 `r[-1]`。极端的当日不应参与构造自己的基线，否则会在最需要报警的那天把 |z| 系统性压低。
- **标准差**：**样本标准差（ddof = 1）**，与 `check_alerts.py` 既有先例一致。
- **门限**：`|z| ≥ 2.0` → `level: "alert"`，否则不列。**比较用未取整的 z**；写进 `latest.json` 的 2 位小数只供展示，不参与判定。

> **🔴 三条约定里只有第一条会翻转结论**（实测 2026-08-28）：`GLD` 在推荐口径下 z = **−2.0533**，余量仅 +0.053σ。按「前 90 个收益率」读，**列出**；按「90 个日历日」读，窗口只剩 **62 根**，z = **−1.9729**，**不列**。同一份数据同一天，只因「日」字的两种读法，简报是 2 条还是 1 条。「不含当日」与「样本/总体」在 W=90 下 8 种组合全部列出 GLD。**若只能写死一条，必须是「前 90 个收益率」。**
>
> 取整陷阱同理：`GLD` −2.0533 与 `PALL` +1.9333 取整后都贴着 2.0，拿取整值比较极易误判。

##### 同一金属两条腿的去重（方案 B，已定）

`{GC=F, GLD}`、`{SI=F, SLV}`、`{PA=F, PALL}`、`{PL=F, PPLT}`、`{HG=F, CPER}` 各成一组，**组内任一腿越线即列该金属**，展示 |z| 最大的那条腿并标注 `leg`。

不这样做就会出现 2026-08-28 的形态：钯金**期货**腿过线（+2.67）而 ETF 腿不过（+1.93），黄金**ETF** 腿过线（−2.05）而期货腿不过（−1.89）——简报于是说「钯金 +6.80%」和「黄金 ETF −3.24%」，却只字不提黄金期货 −2.85%，读者会问「黄金到底动没动」。两条腿的差别主要来自**结算时点**（期货 13:30 ET 结算、ETF 16:00 ET 收盘），不是两件不同的事实。简报回答的是「今天哪种金属异常」，不是「哪个代码异常」。

**两腿冲突时不许静默取一条**（去重只在两腿的 `date` 相同时进行）：

- 两腿**同向**越线 → 取 |z| 大的那条，标 `leg`
- 两腿**反向**同时越线 → **两条都保留**并标 `conflict: true`。这不是噪音，是两个口径在同一天给出相反读数，正是读者该看到的；静默丢掉其中一条等于替他做了一个我们没有依据做的判断
- 两腿 `date` 不同（一条已收盘、一条还没）→ **不去重**，各自按自己的 `asOf` 单独出现

（另两个方案：A 各标的独立判定，实现最简但读者困惑；C 期货优先，**与 §0.1 第 36 行自相矛盾**——那行把「PPLT 永远不会触发任何阈值」当成必须修的缺陷，采纳 C 则 PPLT 本就不该参与筛选，PR-1 的拆分回改就失去了自己陈述的动机。）
...
```json
{
  ...
  "sections": {
    ...
    "metals":    [{ "metal": "钯金", "leg": "PA=F", "price": 1428.60, "changePct": 6.80,
                    "z": 2.67, "window": 90, "level": "alert", "asOf": "2026-08-28" },
                  { "metal": "黄金", "leg": "GLD",  "price": 408.89, "changePct": -3.24,
                    "z": -2.05, "window": 90, "level": "alert", "asOf": "2026-08-28" }],
...
| PR-1b | **期货未完成时段修复（PR-2 验收的前置）**：把 `update_data.py` 取数统一到 `fetch_historical.py` 用的 chart API（`yfinance` 本身也只是同一个 Yahoo 公共 API 的包装，换掉它不损失独立性）；改为重取一段滚动重叠窗口以回补被修订的已完成 bar，并丢弃未完成交易日；定向重建 5 个期货自 2026-02-08 起的数据与日期集合，**不做 730 天全量覆盖** | 仓库日期集合与 chart 一致（周日行清零）；2026-02-08 之后逐行相符；diff 只碰 5 个期货 |
| PR-2 | `generate_briefing.py` + `validate_briefing.py` + 单测。**先不接 workflow**，用 `--date` + `--since-sha` 回放历史日期人工核对 | **2026-08-28 回放（打在冻结 fixture 上，不打实时 Yahoo）：金属段恰好 2 条**——钯金（`PA=F` 腿，+6.80% / z +2.67）与黄金（`GLD` 腿，−3.24% / z −2.05）；其余不列，**特别是 `GC=F`（−2.85% / z −1.89）与 `PALL`（+4.92% / z +1.93）都不列**。测试必须断言**金属与来源腿的身份**，不能只断言条数。原稿的「只出黄金 1 条」每个字段都是错的：那个 −2.22σ 取自污染序列，修正后条数是 2 且没有一条是黄金期货；**2026-08-31 回放：研究段必须出 MiniMax H1（证据日 08-26）——这条是新旧选取键的分水岭，旧规则会丢掉它**；周末回放：`empty: true`；同日两次触发回放：第二次不得丢掉第一次的条目 |
| PR-3a | `update-briefing.yml`（三触发并集）+ `ci-smoke.yml` 两步 + **首次 bootstrap**：首份简报没有既有 `latest.json` 可取 cursor，须由 `workflow_dispatch` 显式传入 owner 确认的 `--since-sha`，产出第一份之后才启用日常触发 | 手动 `workflow_dispatch` 跑通并提交；**造一次 bot 数据提交与一次人署名提交，两条路径都要实证能触发**；产物同步检查生效 |
| PR-3b | 归档页 `briefing/index.html` | zh/en 双语无判断色。**非开跑阻塞项** |
...
**依赖顺序**：规格纠偏 → PR-1 / PR-1b（数据基线）→ PR-2 真实回放验收 → PR-3a → **事实版开跑**。PR-3b / PR-4 可后置，REV 全程可并行。

**「PR-1 未合并前不开始 PR-2」讲的是不要带着污染发布，不是不要并行开发。** PR-2 的框架与合成 fixture 测试可与 PR-1/PR-1b 并行；但**真实金属回放、最终验收、合并与启用 workflow 必须等干净 `main`**。尤其不可在 PR-1b 之前把「08-28 只出黄金」固化成测试契约——那正好是错的那个。

**「开跑」的完成判据不是代码合并**，而是：干净 `main` 上首次手动 dispatch 成功、`latest.json` 过校验、公开站点字节对应本次 `main` SHA，且 bot 数据提交与人署名提交两条触发路径各自实证过一次。**本规格不要求任何 cron 改动**（§2.1）。
...
**Track 1**
- [ ] `historical.json` 中不存在任何单日 |涨跌| > 35% 且不在 `knownSplits[]` 白名单里的记录；造一个假跳变能让 `validate_data.py` FAIL
- [ ] 拆分查询失败时 `update_data.py` **整次不写文件**（不是跳过该标的继续）
- [ ] **PR-1b**：5 个期货不再含未完成时段 bar；仓库日期集合与 Yahoo chart 一致（无周日行）；2026-02-08 之后逐行相符
- [ ] PPLT 90 日日波动率回到 2% 量级（当前 9.80%），两只 ETF 能正常参与阈值筛选

**Track 2**
- [ ] 金属段用收益率 z，汇率段用价格 z；每个 z 值渲染时带窗口标签，每段带独立 as-of
- [ ] **金属段严格按 §4.1.1**：14 个标的同等参与、**前 90 个收益率**（92 根 close）、不含当日、样本标准差、未取整比较、按标的物去重（方案 B）
