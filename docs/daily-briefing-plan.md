...
- **窗口**：**90 根交易日 bar 的收益率，不是 90 个日历日。**
- **窗口位置**：**不含当日**——取当日之前的 90 个收益率（`r[-91:-1]`）。极端的当日不应参与构造自己的基线，否则会在最需要报警的那天把 |z| 系统性压低。
- **标准差**：**样本标准差（ddof = 1）**，与 `check_alerts.py` 既有先例一致。
- **门限**：`|z| ≥ 2.0` → `level: "alert"`，否则不列。**比较用未取整的 z**；写进 `latest.json` 的 2 位小数只供展示，不参与判定。

> **🔴 三条约定里只有第一条会翻转结论**（实测 2026-08-28）：`GLD` 在推荐口径下 z = **−2.0533**，余量仅 +0.053σ。按「90 根交易日 bar」读，窗口 90 根，**列出**；按「90 个日历日」读，窗口只剩 **62 根**，z = **−1.9729**，**不列**。同一份数据同一天，只因「日」字的两种读法，简报是 2 条还是 1 条。「不含当日」与「样本/总体」在 W=90 下 8 种组合全部列出 GLD。**若只能写死一条，必须是「90 根交易日 bar」。**
>
> 取整陷阱同理：`GLD` −2.0533 与 `PALL` +1.9333 取整后都贴着 2.0，拿取整值比较极易误判。

##### 同一金属两条腿的去重（方案 B，已定）

`{GC=F, GLD}`、`{PA=F, PALL}`、`{PL=F, PPLT}`、`{HG=F, CPER}` 各成一组，**组内任一腿越线即列该金属**，展示 |z| 最大的那条腿并标注 `leg`。
...
- [ ] **金属段严格按 §4.1.1**：14 个标的同等参与、90 根**交易日 bar**、不含当日、样本标准差、未取整比较、按标的物去重（方案 B）
