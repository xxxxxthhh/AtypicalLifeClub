...
| Authoring pipeline | `company-research-publishing` skill (git source `~/Documents/codex-private-skills`, deploy by single-file copy; **never `install-local.sh`**) gates full-tier reports: modules + ≥20-row table spine + scenario grid + priced-in + stance v2 fields. Skill repo verified in sync with its origin @ `70b5780` (2026-07-08 review; an earlier "ahead-3 unpushed" note was stale) | Track 3 initiations are standard skill runs; Track 2 needs one skill-side checklist edit. No skill-repo push is pending |
...
- **Availability verified (2026-07-08 spec review, local macOS via `uv run --with yfinance`):** XLU 45.70 / SRVR 30.89 / COPX 74.63 / QQQ 709.43 / SMH 581.45 all return usable daily closes; XLRE (44.89) works as the SRVR fallback. Note the runner constraint: system `python3` is 3.9.6 without yfinance — these scripts run via `uv run` (PEP 723 inline metadata) locally, `pip install yfinance` + Python 3.11 in the GitHub cron. If the cron/CI environment differs from where this probe ran, re-verify there before building (see §8 risk 3).
...
| PR | Content | Gate |
|---|---|---|
| PR-1 | `benchmarks.json` + resolution in `update_verdicts.py` + validator (incl. `benchmarkRationale` requirement, scoped self-benchmark ban) + grandfather relabel + **minimal ledger-page compatibility (§2.3 PR-1 items)** (symbol availability already probed, §2.1) | all validators green; verdicts regenerate with per-entry `benchmarkSymbol`; page shows per-row benchmark and never averages across benchmarks; unit tests for resolution order + rationale enforcement + the self-ban fixture matrix: `copx-2026` passes **via its override** (the only chain report whose `priceSymbol` collides with a layer default — if it FAILs, the carve-out was applied to the wrong field), `vrt-2026` passes via override, a synthetic chain report resolving to its own symbol FAILs, and `smh-2026` (benchmark-only sleeve) is never scanned |
