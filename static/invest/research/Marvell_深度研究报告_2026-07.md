...
以下是估值上下文，不是建议。`prices.json` 记录的价格锚点为 **2026-07-02 的 $245.29**。
...
| EV/FY2028E Sales | 约 13x（用 $16.5B） | 只有兑现后才可解释，但仍高 |
| FCF yield | TTM FCF 约 $1.66B，对 EV 不到 1% | 当前不是现金流估值故事 |

**情景表：**

| 情景 | 驱动假设（custom ASIC ramp / 光互连 attach / 利润率 / 倍数区间） | 相对 $245.29 日期价格的估值含义 | 概率权重 |
|------|--------------------------------------------------------------------------------|----------------------------------|----------|
...
**已经定价的预期与差距：** 在 **$245.29**、约 **$215.7B** EV、按公开 **$11.5B** 框架约 **19x FY2027E sales**、按 **$16.5B** 框架约 **13x FY2028E sales** 上，市场已经假设 custom ASIC 与 optical 故事会转成大很多的收入基数。反推 sales multiple 很苛刻：若 AI platform 半导体倍数三年内向 **8-10x sales** 正常化，且要求约 **8%** 年回报，MRVL 需要从 FY2027 框架开始约 `((19 / 10)^(1/3) x 1.08 - 1) ≈ 34%` 到 `((19 / 8)^(1/3) x 1.08 - 1) ≈ 44%` 的年收入增长，并且利润率同步改善。我们的基准情景没有足够安全边际越过这条线。因此即使上行情景真实，预期差距仍为负。
...
预期差距为负：在 $245.29、约 19x FY2027E sales 上，市场已经要求 FY2027/FY2028 收入 ramp 顺利兑现并继续复合；我们的基准情景承认战略期权，但不足以用足够安全边际越过隐含的 34-44% 年增长门槛。
