# Coinbase Global (COIN) Deep Research Report

**[2026-02-13 Update] Updated with Q4 2025 earnings data (released 2/12 after market close). Q4 revenue $1.78B missed estimates; net loss of $667M ended 8-quarter profitability streak. FY2025 revenue confirmed at $7.18B. Old data marked with ~~strikethrough~~.**

**Date:** February 11, 2026 (Updated: February 13, 2026)
**Analyst:** AI-Assisted Research
**Disclaimer:** This report does not constitute investment advice. Please conduct your own due diligence.

---

## Executive Summary

**One-Line Thesis:** Coinbase is the monopolistic gateway to U.S. compliant crypto infrastructure, transitioning from a cyclical exchange to a diversified financial platform, but its fundamental dependence on crypto market sentiment remains unchanged.

**Investment Verdict:**
- Lean: Cautiously Bullish | Conviction: Medium
- Key Catalyst: ~~Q4 2025 earnings (releasing 2/12)~~ Q4 2025 earnings released 2/12 — missed estimates, stock -7.9% AH; crypto regulatory legislation progress (GENIUS Act / CLARITY Act), BTC price trajectory

**Quick Stats:**

| Metric | Value |
|--------|-------|
| Market Cap | ~$40-50B |
| Price (2026.2.11) | ~$150-165 |
| 52-Week High/Low | $444 / $147 |
| P/E (TTM) | ~15x |
| P/E (Forward) | ~30x |
| FY2024 Revenue | $6.56B |
| ~~FY2025E Revenue~~ | ~~$7.2B (+10%)~~ |
| FY2025 Revenue (Actual) | $7.18B (+9% YoY) |
| Q4 2025 Revenue | $1.78B (missed est. $1.85B) |
| Q4 2025 EPS | $0.66 (missed est. $1.05) |
| Q4 2025 Net Loss | -$667M |
| FY2025 Trading Volume | $5.2T (+156% YoY) |
| FY2024 FCF | $2.56B |
| Cash | $8B+ |
| Long-term Debt | $5.93B |

---

## 1. Business Overview

### What Is Coinbase

Founded in 2012 and publicly listed on Nasdaq via direct listing in 2021, Coinbase is the largest regulated cryptocurrency exchange in the United States. In May 2025, it was added to the S&P 500 — a historic milestone for a crypto-native company.

### How It Makes Money

Coinbase's revenue mix is undergoing a structural transformation:

**Transaction Revenue (~55-60%):**
- Retail trading fees (core profit driver, significantly higher take rates than institutional)
- Institutional trading (81% of volume, but lower fees)
- Derivatives trading (significantly expanded post-Deribit acquisition)

**Subscription & Services Revenue (~40%):**
- USDC interest income (largest single item, ~~$355M in Q3 2025~~ $364M in Q4 2025; FY2025 stablecoin revenue $1.35B)
- Staking rewards
- Custody fees (custodian for 9 of 11 spot Bitcoin ETFs)
- Coinbase One subscription
- Base chain-related revenue

**Q3 2025 Revenue Breakdown:**
- Total Revenue: $1.87B (QoQ +25%, YoY +55%)
- Transaction Revenue: $1.05B (QoQ +37%)
- Subscription & Services: $747M (QoQ +14%, YoY +34%)

**Q4 2025 Revenue Breakdown (Actual):**
- Total Revenue: $1.78B (QoQ -5%, YoY -21.6%) — missed analyst estimate of $1.85B
- Transaction Revenue: $983M (QoQ -6%)
- Subscription & Services: $727M (QoQ -3%)
- Consumer Spot Volume: $56B; Institutional Spot Volume: $215B
- Adjusted EBITDA: $566M
- Net Loss: -$667M (investment-related losses from crypto portfolio; ended 8-quarter profitability streak)

**FY2025 Full Year:**
- Total Revenue: $7.18B (+9% YoY)
- Total Trading Volume: $5.2T (+156% YoY)
- Crypto Trading Volume Market Share: 6.4% (doubled YoY)
- Total Operating Expenses: $5.75B (+35% YoY)

### Key Business Lines

**1. Base Chain (Layer 2)**
- Ethereum-based L2 network; Q4 2025 stablecoin transaction volume reached $3 trillion (+121% QoQ)
- Surpassed Ethereum and Tron to become the largest network by stablecoin volume
- $3.7B USDC held on Base

**2. Deribit Acquisition ($2.9B, completed August 2025)**
- World's largest crypto options exchange
- 2024 trading volume exceeded $1 trillion, $30B open interest
- ~~Contributed $52M revenue in Q3 2025~~
- Deribit achieved all-time high quarter in Q4 2025 under Coinbase ownership
- Makes Coinbase a comprehensive platform spanning spot, futures, perpetuals, and options
- Coinbase now the global leader in crypto derivatives by open interest and options volume

**3. USDC Ecosystem**
- ~~$73.7B in circulation (Q3 2025)~~
- Average USDC market capitalization: $76.2B (FY2025); average USDC balances on Coinbase: $17.8B
- Coinbase receives 100% interest income on on-platform USDC + 50% off-platform (revenue share with Circle)
- ~~Coinbase holds ~23% of total USDC supply~~
- FY2025 stablecoin revenue: $1.35B (within subscription & services)

---

## 2. Industry & Competitive Position

### Crypto Exchange Market Landscape

| Exchange | Global Spot Market Share | Positioning |
|----------|------------------------|-------------|
| Binance | ~38-42% | Global dominant, most comprehensive products |
| Coinbase | ~~~5-6%~~ 6.4% (doubled YoY in 2025) | U.S. compliance leader |
| Kraken | ~3-4% | Strong in EUR/USD pairs |
| OKX/Bybit | ~5-8% each | Primarily Asian markets |

### Coinbase's Moat Assessment

**🏰 Strong Moat Factors:**

1. **Regulatory Compliance Barrier**
   - Only publicly listed major crypto exchange in the U.S.
   - S&P 500 constituent status brings institutional trust
   - Custodian for 9/11 Bitcoin ETFs — an extremely high trust endorsement
   - New York State Trust Company license

2. **Network Effects (Moderate)**
   - ~~9.3M monthly active users (Q3 2025)~~
   - ~~$516B in platform assets~~
   - Custodian for 12% of global crypto market cap
   - FY2025: all-time highs in total trading volume ($5.2T), crypto market share (6.4%), subscription revenue, and Coinbase One subscribers

3. **Brand & Trust**
   - Default on-ramp for U.S. retail investors
   - 12-year operating history with no major security incidents

**🏚️ Weak Moat Factors:**

1. **No Pricing Power on Trading Fees**
   - Binance fees significantly lower than Coinbase
   - Zero-commission trend may spread
   - Retail trading fee rates under persistent pressure

2. **Inherent Cyclicality of Crypto Markets**
   - In bull markets everyone's a genius; in bear markets volume can drop 80%+
   - 2022 lesson: revenue crashed from $7.8B to $3.1B

**Overall Moat Rating: Moderate-to-Strong (🏠→🏰 transitioning)**

Coinbase's moat lies not in trading itself, but in its monopolistic position as U.S. compliant crypto infrastructure. As subscription revenue share increases and ETF custody creates lock-in effects, the moat is deepening.

---

## 3. Financial Analysis

### Financial Health Matrix

| Metric | Current | 3Y Ago (2022) | Trend | Grade |
|--------|---------|---------------|-------|-------|
| Revenue Growth | ~~+10% YoY~~ +9% YoY (FY2025 confirmed $7.18B) | -57% YoY | ↗️ Recovery | B |
| Gross Margin | ~85% | ~75% | ↗️ | A |
| Operating Margin | ~~~24% (Q3'25)~~ Q4'25: negative (net loss -$667M) | -108% (2022) | ↗️↘️ Volatile | B- |
| ROIC | ~~~15-20%~~ Declining (Q4 net loss) | Negative | ↗️↘️ | C+ |
| FCF | $2.56B (2024) | -$2.3B (2022) | ↘️ Declining in 2025 | B- |
| Cash | $8B+ | ~$5B | ↗️ | A |
| Long-term Debt | $5.93B | ~$3.4B | ↗️ Increasing | C+ |
| FY2025 OpEx | $5.75B (+35% YoY) | — | ↗️ Rising fast | C |

### Key Financial Observations

**Positive Signals:**
- FY2024 revenue $6.56B, net income $3.2B (vs only $95M in 2023) — profitability explosion
- FY2025 revenue confirmed $7.18B (+9% YoY), with all-time highs in trading volume ($5.2T) and market share (6.4%)
- Cash reserves $8B+, ample liquidity
- Subscription revenue share increased from ~25% (2022) to ~40%, improving revenue quality
- ~85% gross margin reflects high-leverage platform economics
- Deribit achieved all-time high quarter in Q4 2025
- First to market with 24/7 U.S. perpetual-style futures; U.S. derivatives market share 4x YoY

**Warning Signals:**
- ⚠️ SBC at $2.1B/year — 29% of revenue, severely diluting shareholder value
- ⚠️ ~~2025 FCF likely declining — Q3 2025 quarterly FCF was negative $785M~~ Q4 2025 net loss of -$667M confirmed; ended 8-quarter profitability streak (investment-related losses from crypto portfolio)
- ⚠️ FY2025 operating expenses surged to $5.75B (+35% YoY) — cost growth outpacing revenue growth
- ⚠️ Long-term debt increased to $5.93B — includes $2.6B convertible notes
- ⚠️ Highly cyclical revenue — 2022 lesson: revenue can halve and halve again
- ⚠️ Forward P/E ~30x vs TTM P/E ~15x — market expects significant earnings decline in 2026
- ⚠️ Q1 2026 guidance: subscription & services revenue $550-630M (below Q4's $727M)

### Red Flag Check

| Red Flag | Status | Notes |
|----------|--------|-------|
| Revenue growth > Cash flow growth | ⚠️ Watch | 2025 FCF declining while revenue grows |
| Excessive SBC | 🔴 Confirmed | $2.1B/yr, 29% of revenue |
| Insider selling | 🟡 Watch | Armstrong consistently selling |
| Audit/Accounting issues | ✅ None | Big 4 audit, no anomalies |
| Related party transactions | ✅ None | No material related-party deals |
| Goodwill concentration | ✅ Normal | Monitor post-Deribit |

---

## 4. Management Assessment

### Brian Armstrong (CEO & Co-Founder)

**Background:** Former Airbnb engineer, founded Coinbase in 2012, led the company from zero to S&P 500.

**Strengths:**
- Clear long-term vision: exchange → financial platform → crypto infrastructure
- Decisive cost-cutting in 2022 bear market (~20% layoffs), saved the company
- Pursued compliance route, confronted SEC head-on and ultimately prevailed
- Clear 2026 strategy: "everything exchange" (crypto + equities + commodities + derivatives)
- 2026 three priorities outlined on Q4 earnings call: (1) Growing the "Everything Exchange", (2) Scaling stablecoins & payments, (3) Increasing on-chain economic activity

**Weaknesses:**
- Consistent stock selling — sold $291.8M at 2021 listing, continued selling via 10b5-1 plans
- Facing insider trading lawsuit from 2021 listing (Delaware court allowed case to proceed Jan 2026)
- Dual-class share structure, Armstrong maintains voting control via Class B shares

**Ownership:** ~19% economic interest, voting control through Class B shares

**Management Grade: B**
Strong execution, correct strategic direction, but persistent selling and dual-class structure are governance negatives.

---

## 5. 🐂 Bull Case (Steel-Manned)

**Core Thesis:** Coinbase is the "picks and shovels" play on the U.S. crypto economy — regardless of which coin rises or falls, as long as the crypto market is active, Coinbase profits. Current valuation fully reflects bear market expectations, while multiple structural growth engines remain unpriced.

**Supporting Evidence:**

**1. Revenue Structure Transformation — No Longer Just an Exchange**
- Subscription & services revenue share grew from 25% → 40%, still increasing
- USDC interest income is "passive income" — as long as rates don't go to zero, this cash keeps flowing
- Base chain ecosystem exploding (Q4 stablecoin volume $3T), potential for significant on-chain revenue
- Post-Deribit integration, derivatives revenue becomes a new growth engine

**2. Regulatory Tailwind Arriving**
- SEC lawsuit dismissed February 2025 — the biggest sword of Damocles removed
- Trump administration crypto-friendly, SEC formed Crypto Task Force
- ~~Crypto legislation advancing (CFTC digital commodities bill)~~
- GENIUS Act (stablecoin framework) signed into law July 18, 2025 — landmark legislation requiring 1:1 backing, monthly disclosures; implementing rules due July 2026
- CLARITY Act (market structure) passed House July 2025, Senate passage anticipated in 2026
- SEC & CFTC launched "Project Crypto" — joint token taxonomy initiative for 2026
- Compliant exchanges will capture the greatest regulatory clarity dividend

**3. Institutionalization Wave**
- Custodian for 9/11 Bitcoin ETFs — this is a "selling water to miners" business
- Institutional volume at 81% and still growing
- Banks recommending BTC allocation (BofA suggests 4%)
- 401(k) may include BTC ETFs — massive incremental capital

**4. Valuation Has Significantly Corrected**
- Down from $444 to ~$150, a 65%+ decline
- TTM P/E ~15x, well below peer average of 32.6x
- If crypto market recovers in 2026, earnings leverage is enormous

**Key Assumptions:**
- BTC maintains $80K+ levels in 2026
- Interest rates don't decline sharply (protecting USDC interest income)
- Crypto regulatory legislation makes substantive progress in 2026
- Deribit integration proceeds smoothly, derivatives revenue grows

**Upside Scenario:** If BTC returns to $100K+, trading volumes recover, Coinbase annualized revenue could reach $10B+, stock price back to $300-400 range.

---

## 6. 🐻 Bear Case (Steel-Manned)

**Core Thesis:** Coinbase is fundamentally still a crypto cycle stock. The current "diversification" narrative masks its root dependence on crypto market sentiment. In a crypto winter, all growth stories freeze.

**Supporting Evidence:**

**1. The Brutal Reality of Crypto Cycles**
- 2022 lesson: revenue from $7.8B → $3.1B (-60%), net loss of $2.6B
- BTC down from $126K peak to ~$70K (-44%); if it continues falling, trading volumes will further shrink
- Retail trading revenue (highest-margin business) can drop 80%+ in bear markets
- Forward P/E ~30x indicates market already expects significant earnings decline

**2. SBC Is an Invisible Value Destroyer**
- $2.1B/year SBC is 29% of revenue — this is not "free"
- Share count grew from ~190M at listing to ~263M (+38% dilution)
- On SBC-adjusted real earnings, valuation is far less cheap than it appears
- Management itself is selling — Armstrong consistently reducing via 10b5-1 plans

**3. Competitive Pressure Cannot Be Ignored**
- Binance global share 40%+ vs Coinbase 5-6%
- Kraken planning 2026 IPO, will directly compete for U.S. compliant market
- Traditional brokers (Robinhood, Schwab) entering crypto trading
- DEXs continuously eroding share
- Long-term downward trend in trading fee rates is irreversible

**4. USDC Interest Income Fragility**
- If the Fed cuts rates significantly, USDC interest income will shrink materially
- This is Coinbase's largest "stable" revenue source, but entirely dependent on rate environment
- Tether (USDT) remains the absolute dominant stablecoin

**5. Regulatory Risk Not Fully Eliminated**
- California DFAL Act effective July 2026, compliance costs may surge
- Nevada enforcement action against prediction markets (Feb 2026)
- State-level regulatory fragmentation risk
- Customer class action lawsuits still ongoing
- GENIUS Act implementing rules due July 2026 — compliance burden for stablecoin operations
- Stablecoin rewards remain a contentious flashpoint — banking industry lobbying against crypto platforms offering yield
- Coinbase temporarily withdrew support for Senate CLARITY Act over stablecoin reward limitations

**Downside Scenario:** If BTC falls to $40-50K and crypto enters a deep bear market, Coinbase revenue could retreat to $4-5B, net income shrinks dramatically, stock price could fall to $80-100.

---

## 7. ❓ Key Uncertainties

### What We Don't Know

**1. Where We Are in the Crypto Cycle**
- BTC dropped from $126K to ~$70K — is this a bull market pullback or the start of a bear market?
- If it's a pullback, current prices are an excellent entry; if bear market is starting, significant downside remains
- When we'll know more: Watch whether BTC holds $60K support

**2. Interest Rate Path**
- Fed rate cuts in 2026 directly impact USDC interest income
- Every 100bp cut could reduce USDC interest income by $200-300M/year
- When we'll know more: Watch Fed March and June meetings

**3. Deribit Integration Results**
- ~~Can the $2.9B acquisition deliver expected synergies?~~
- Q4 2025 was Deribit's all-time high quarter under Coinbase ownership — early integration positive
- Can derivatives revenue become a stable income source?
- Coinbase now first to market with 24/7 U.S. perpetual-style futures; U.S. derivatives market share 4x YoY
- When we'll know more: H1 2026 quarterly earnings (full-year derivatives revenue breakdown)

**4. Crypto Regulatory Legislation**
- ~~Can federal crypto legislation pass in 2026?~~
- GENIUS Act (stablecoin) already signed into law July 2025; implementing rules due July 2026
- CLARITY Act (market structure) passed House, Senate vote pending in 2026
- SEC & CFTC "Project Crypto" token taxonomy initiative underway
- If passed, Coinbase as a compliant platform captures enormous upside
- If not, state-level fragmented regulation increases costs
- Stablecoin rewards remain a flashpoint — Coinbase withdrew support for Senate CLARITY Act version over this issue
- When we'll know more: 2026 Congressional legislative progress, GENIUS Act implementing rules (July 2026)

### Thesis-Breaking Events

- If BTC falls below $40K and stays depressed → Bull case invalidated
- If crypto legislation passes and BTC maintains $80K+ → Bear case invalidated
- If a major security incident or regulatory penalty occurs → All theses need reassessment

---

## 8. Valuation Analysis

### Multi-Dimensional Valuation

| Method | Value/Share | Key Assumptions | Confidence |
|--------|-------------|-----------------|------------|
| P/E (Peer Comps) | $250-320 | Peer average 32.6x P/E | Medium |
| P/E (Cycle-Adjusted) | $120-180 | Cycle-average earnings, 20x | Medium-High |
| EV/EBITDA | $200-280 | 15-20x EV/EBITDA | Medium |
| Analyst Consensus | $290-342 | 32 analyst average | Medium |
| Bear Scenario | $80-120 | Deep crypto bear market | Low |
| Bull Scenario | $350-450 | BTC $100K+, volume recovery | Low |

**Current Price: ~$150-165**

**Key Observations:**
- TTM P/E ~15x significantly below peer average 32.6x — appears cheap on the surface
- But Forward P/E ~30x indicates market expects significant earnings decline
- The real question: where will 2026 earnings land?
- If crypto market recovers, significant upside from current prices
- If crypto market continues deteriorating, further downside risk remains

**Note:** Valuation serves as a reference framework only, not a buy/sell recommendation. Coinbase's valuation is highly dependent on crypto cycle judgment.

---

## 9. Catalysts & Timeline

### Near-Term (0-3 Months)

| Catalyst | Timing | Impact |
|----------|--------|--------|
| ~~Q4 2025 Earnings~~ | ~~2026.2.12~~ Released 2/12 | ~~High — volume and guidance are key~~ Result: missed estimates, EPS $0.66 vs $1.05 est., revenue $1.78B vs $1.85B est., stock -7.9% AH |
| Q1 2026 Guidance | Provided 2/12 | Sub & services revenue guided $550-630M (below Q4's $727M) |
| BTC Price Trajectory | Ongoing | High — directly impacts trading revenue |
| Fed Meeting | 2026.3 | Medium — rate path affects USDC income |

### Medium-Term (3-12 Months)

| Catalyst | Timing | Impact |
|----------|--------|--------|
| Crypto Regulatory Legislation | 2026 | High — GENIUS Act implementing rules due July 2026; CLARITY Act Senate vote pending |
| Deribit Integration Results | 2026 H1 | Medium-High — Q4 2025 was all-time high quarter for Deribit; derivatives revenue growth validation |
| California DFAL Act | 2026.7 | Medium — compliance cost impact |
| Kraken IPO | 2026 H1 | Medium — competitive landscape shift |
| GENIUS Act Implementing Rules | 2026.7.18 | Medium — stablecoin regulatory framework finalized |

### Long-Term (1-3 Years)

| Catalyst | Timing | Impact |
|----------|--------|--------|
| Next BTC Cycle | 2026-2028 | High — decisive factor |
| "Everything Exchange" Strategy | 2026-2027 | Medium — equities/commodities expansion |
| 401(k) BTC ETF Inclusion | 2026-2027 | High — massive incremental capital |
| Base Chain Ecosystem Maturation | 2026-2028 | Medium — on-chain revenue potential |

---

## 10. Conclusion

### Overall Assessment

Coinbase is a unique investment — both a "picks and shovels" play on crypto and a fintech platform in transformation.

**Core Tension:**
- Bull logic: Compliance monopoly + revenue diversification + 65% valuation pullback = excellent entry
- Bear logic: Crypto cycle stock + SBC dilution + earnings decline = value trap

**Our Lean: Cautiously Bullish, But Discipline Required**

Rationale:
1. SEC dismissal + S&P 500 inclusion = institutional risk dramatically reduced
2. 40% subscription revenue share means revenue floor is much higher than 2022
3. ETF custody + Deribit + Base chain = multiple growth engines
4. But SBC problem is serious, and management selling is not a good signal
5. Crypto cycle uncertainty is the biggest risk

### Position-Building Considerations

Given you're gradually building a position, a few thoughts:

1. Dollar-cost averaging is the right approach — crypto cycle uncertainty is too high for concentrated entry
2. ~~Watch tomorrow's Q4 earnings closely — the most important near-term catalyst, may cause short-term volatility~~ Q4 earnings released 2/12: missed estimates, stock -7.9% AH. Q1 2026 sub & services guidance $550-630M signals continued softness
3. Set a mental stop-loss — if BTC breaks below $50K, Coinbase fundamentals deteriorate significantly
4. Monitor SBC trends — if SBC as % of revenue doesn't decline, long-term shareholder value erodes
5. Position sizing — given crypto's high volatility, consider keeping Coinbase at no more than 5-8% of portfolio

### Key Monitoring Metrics

- BTC price and trading volume trends
- Subscription & services revenue QoQ growth
- SBC as percentage of revenue
- Deribit derivatives revenue contribution
- Fed rate decisions (USDC income impact)
- Crypto regulatory legislation progress

---

## Appendix

### Data Sources
- Coinbase official earnings reports and Investor Relations
- SEC EDGAR public filings
- Analyst reports (JPMorgan, Citi, Zacks)
- CoinGecko, The Block market data
- Various financial media coverage

### Peer Comparison

| Metric | Coinbase | Robinhood | CME Group |
|--------|----------|-----------|-----------|
| FY2024 Revenue | $6.56B | ~$2.4B | ~$6.1B |
| Operating Margin | ~34% | ~25% | ~55% |
| P/E (TTM) | ~15x | ~25x | ~25x |
| Crypto Revenue % | ~100% | ~20% | ~5% |
| Revenue Diversification | Medium | High | High |

### Key Assumptions
- BTC 2026 average price: $70-100K (base case)
- Fed cuts rates 1-2 times in 2026 (base case)
- ~~Crypto legislation makes progress but doesn't pass in 2026 (base case)~~
- GENIUS Act implementing rules finalized by July 2026; CLARITY Act Senate vote pending (base case)
- ~~Deribit annualized revenue contribution $200-400M (base case)~~
- Deribit Q4 2025 was all-time high quarter; annualized revenue contribution tracking toward upper end of $200-400M range (base case)
- Q1 2026 subscription & services revenue: $550-630M (management guidance)
- FY2025 OpEx growth (+35% YoY) moderates in 2026 (base case)
