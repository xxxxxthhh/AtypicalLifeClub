# Vistra (VST) Deep Research Report

Coverage date: 2026-07-04
Last updated: 2026-08-11
Ticker: NYSE: VST
Disclaimer: This report is for informational and research purposes only. It does not constitute investment advice. Please conduct your own due diligence.

---

## Executive Summary

> **Framework note:** Vistra pairs with Constellation in the AI-infrastructure book's power layer. It is the higher-beta nuclear-plus-gas dashboard, testing whether data centers need a mixed stack of clean baseload, dispatchable gas, capacity-market exposure, and balance-sheet risk.

**One-line thesis:** VST is the nuclear-plus-gas operating-leverage name in the AI power constraint, and the current v5 stance is **cautious / medium conviction**. 2026Q1 revenue was $5.64B, net income was $1.03B, operating income was $1.50B, and FCF was $316M. Meta's 20-year nuclear agreements cover Perry, Davis-Besse, and Beaver Valley uprates, with public reports framing about 2.2GW. The Cogentrix transaction is framed at about a $4B net purchase price, while public seller-side reporting has also framed it near $4.7B total consideration, and adds about 5,500MW of gas capacity with expected mid-to-late 2026 close.

> **Old price-check note — stale as written, superseded by the re-adjudication below.** ~~2026-07-31 price check: VST closed at $142.81 on 2026-07-29 versus the $151.05 anchor, a -5.5% move, so the dated anchors below remain accurate within that margin and the valuation frame, scenario grid, stance and conviction are unchanged; no edit was required.~~

> **2026-07-31 stance re-adjudication at the 2026-07-30 close.** Owner ruling (2026-07-31): a published stance and its rationale must hold at the CURRENT price. VST closed at **$148.62** on **2026-07-30**, **-1.6%** from the old $151.05 anchor of 2026-07-02. On about 337.0M shares and about $19.3B of net debt, market cap is about **$50.1B** (old about $50.9B) and enterprise value about **$69.4B** (old about $70.2B); on the same TTM GAAP EPS base near $6.57 that is about **22.6x** TTM GAAP P/E (old about 23x), and EV/TTM sales is about **3.6x**, unchanged to one decimal place. This is the honest low-drift outcome: a 1.6% move does not shift the EV/sales multiple at all at the stated precision, and moves the P/E by less than half a turn. The mildly negative expectation gap rested on Meta PPA economics, Cogentrix closing and converting about 5,500MW, PJM/Texas capacity pricing and leverage — every one of which is an execution or contracting question that a 1.6% price move leaves exactly where it was. **Stance stays cautious (medium conviction), with the stated multiples confirmed rather than merely carried over.**

> **2026-08-07 radar update: Q2 2026 has landed, superseding the "2026Q1" fact base used in the quick-stat table below and in §3.** Vistra reported Q2 2026 on **2026-08-07** (8-K Item 2.02, Exhibit 99.1). Operating revenues were **$4,017M** (Q2 2025: $4,250M) and GAAP net income **$305M** (Q2 2025: $327M), the decline driven primarily by a **$488M** year-over-year increase in unrealized mark-to-market losses on derivatives; the quarter carries a **$472M** unrealized hedge loss the company says is expected to settle in future years. **Ongoing Operations Adjusted EBITDA was $1,767M**, up **$418M** (more than 30%) on Q2 2025, which the company attributes to higher realized energy and capacity prices *and* three months of contribution from the plants acquired from Lotus — so the increase is not purely organic pricing. By segment: Texas **$311M** (Q2 2025: $142M), East **$642M** ($418M), Retail **$773M** ($756M), West **$68M** ($49M). **2026 guidance was reaffirmed** at Ongoing Operations Adjusted EBITDA of **$6,800-7,600M** and Ongoing Operations Adjusted FCFbG of **$3,925-4,725M**, and the previously announced **2027** Ongoing Operations Adjusted EBITDA midpoint opportunity range of **$7.4-7.8B** was restated; both exclude the pending Cogentrix acquisition and the signed Meta PPAs, "part of which are expected to contribute to our Adjusted EBITDA in 2027." As of **2026-08-03** the company was hedged about **100%** of expected 2026 generation volumes, about **94%** for 2027 and about **72%** for 2028. Vistra **received FERC approval of the pending Cogentrix acquisition**; the deal has not closed. The release also lists the announcement of **Helix Digital Infrastructure** alongside KKR, the Kuwait Investment Authority and NVIDIA, with **an initial commitment from Vistra of up to $1.0 billion** and Vistra as Helix's preferred power provider (Helix itself was announced publicly by KKR on 2026-06-11). Capital return: **$709M** of first-half repurchases, about $6.5B cumulatively since November 2021, about **336M** shares outstanding at 2026-08-03, and about $1.2B of remaining authorization. Total available liquidity was about **$6,295M** at 2026-06-30, including **$435M** of cash.
>
> **This is a monitoring-only update; no valuation has been recomputed.** Price, market cap, enterprise value, the multiples, the scenario grid and the stance (**cautious / medium conviction**) all remain anchored to the **2026-07-30 close of $148.62**, with the historical price/date boundary set out above. Readings: `gas-capacity-market` is **within** — the non-nuclear leg confirmed, with the Lotus-contribution caveat stated; `cogentrix-integration` is **unclear**, because that item's positive branch requires an on-time close *and* conversion of roughly 5,500MW into data-center/capacity-market value while its negative branch requires delays or missed synergies, and neither branch fired — FERC approval is progress toward a closing condition, not the condition itself; `meta-nuclear-ppa` stays **unclear** because the release adds delivery timing only and no uprate capacity, price or regulatory detail; `leverage-fcf-discipline` is **unclear** because the release carries no balance sheet and discloses no Q2 net debt or net-debt/EBITDA, so the prior "Q1 net debt about $19.3B" reading is not superseded by a disclosed Q2 figure and the 10-Q had not been filed as of 2026-08-07. Source: [Vistra Q2 2026 earnings release, 8-K Exhibit 99.1 (2026-08-07)](https://www.sec.gov/Archives/edgar/data/1692819/000169281926000017/vistra-20260630xearningsre.htm).

> **2026-08-11 radar update: the Q2 10-Q has landed and closes the open balance-sheet question left by the 2026-08-07 release.** Vistra filed its Q2 2026 10-Q on **2026-08-10** for the period ended 2026-06-30. Balance-sheet lines at **2026-06-30 against 2026-03-31**, both taken from Vistra 10-Q filings: long-term debt including amounts due currently **$19,595M** vs $19,163M; long-term debt before unamortized premiums, discounts and issuance costs **$19,764M** vs $19,332M; short-term borrowings **$0** vs $0; accounts receivable financing **$300M** vs $750M; forward repurchase obligation due currently **$613M** vs $641M; cash and cash equivalents **$435M** vs $634M. Those debt-like lines less cash total **$20,073M** at 2026-06-30 against **$19,920M** at 2026-03-31, up about **0.8%** quarter over quarter. First-half cash provided by operating activities was **$2,222M** against $1,171M in H1 2025, with capital expenditures including nuclear fuel purchases and LTSA prepayments of **$1,572M** against $1,458M.
>
> **This remains a monitoring-only update; no valuation has been recomputed.** Price, market cap, enterprise value, the multiples, the scenario grid, the stance (**cautious / medium conviction**) and the EV inputs stated elsewhere in this report all remain anchored to the **2026-07-30 close of $148.62**. `leverage-fcf-discipline` moves from **unclear** to **within**, graded strictly against that item's own trigger — Cogentrix, capex, buybacks or fuel/operating volatility raising net-debt/EBITDA and FCF pressure. Debt-like obligations net of cash rose only about 0.8% quarter over quarter, while Ongoing Operations Adjusted EBITDA rose more than 30% year over year ($1,767M in Q2) and first-half operating cash flow nearly doubled year over year; taken together, the filing does not show the meaningful leverage or FCF pressure required by the negative branch. This is **not** a formal quarter-over-quarter net-debt/EBITDA calculation: Vistra presents no "net debt" subtotal, and the EBITDA comparison is year over year rather than quarter over quarter. The Cogentrix acquisition has **not closed**, so that leg of the trigger is untested and none of its consideration appears on this balance sheet; first-half repayments of long-term debt of $3,859M include refinancing and are not a net-debt movement. Source: [Vistra Q2 2026 Form 10-Q (filed 2026-08-10)](https://www.sec.gov/Archives/edgar/data/1692819/000169281926000019/vistra-20260630.htm); prior-quarter comparatives from [Vistra Q1 2026 Form 10-Q (filed 2026-05-08)](https://www.sec.gov/Archives/edgar/data/1692819/000169281926000014/vistra-20260331.htm).

At $148.62 on Jul 30, 2026, VST represented about $50.1B of market cap and about $69.4B of enterprise value, including about $19.3B net debt. It is cheaper than CEG on enterprise value, but riskier because it has more merchant power, gas, leverage, and transaction integration exposure. The report's job is to test whether AI power demand spreads from named nuclear PPAs into gas capacity, PJM/Texas capacity value, and dispatchable generation. If Meta execution, Cogentrix integration and capacity prices validate, VST has strong operating leverage. If gas prices, regulation, debt, or capacity markets roll over, the downside is faster too.

| Quick stat | Current readout |
|------------|-----------------|
| Share price | $148.62, Jul 30 2026 close, Yahoo/yfinance basis (historical valuation anchor) |
| Market cap / shares | About $50.1B; about 337M shares (2026-07-30 anchor) |
| Enterprise value | About $69.4B, including about $19.3B net debt (2026-07-30 anchor) |
| Latest reported period | Q2 2026 monitoring update; 2026-07-30 valuation frame retained |
| Q2 2026 revenue / net income | $4,017M / $305M (2026-08-07 release) |
| Q2 2026 Ongoing Ops Adjusted EBITDA | $1,767M, up $418M YoY |
| Q1 revenue / net income | $5.64B / $1.03B |
| Q1 operating income / FCF | $1.50B / $316M |
| Meta nuclear agreements | 20-year agreements; Perry, Davis-Besse, Beaver Valley uprates; public reports frame about 2.2GW |
| Cogentrix | About $4B net purchase price; public seller-side reporting also frames it near $4.7B total consideration; about 5,500MW gas capacity, expected mid-to-late 2026 close |
| FCF context | TTM FCF about $1.80B in the existing snapshot |
| Chain role | Cautious / medium-conviction power dashboard across nuclear PPAs, gas capacity, capacity markets, leverage |

## 1. Business And Chain Role

Vistra is an integrated generation and retail power platform with nuclear, gas, coal / transition assets, and retail power capabilities. AI data centers do not need only "green power"; they need dispatchable capacity that can come online quickly and match load volatility. VST therefore carries more merchant power and gas-capacity beta than CEG.

The chain role is to prevent the power layer from becoming a single clean-nuclear story. If real data-center growth requires a mixed stack, VST should show it through Meta nuclear agreements, Cogentrix gas capacity, PJM/Texas capacity prices, and cash-flow support for leverage. If VST weakens while CEG holds, the power thesis may be real but narrower and cleaner than the market's broad trade.

| Asset / revenue mix | Current readout | AI-infra relevance |
|---------------------|-----------------|--------------------|
| Nuclear fleet | Meta 20-year agreements cover Ohio/Pennsylvania nuclear assets | Low-carbon baseload and hyperscaler PPA validation |
| Nuclear uprates | Perry, Davis-Besse, and Beaver Valley uprates are central to the Meta agreement frame | Existing assets can be repriced without waiting for new nuclear |
| Cogentrix gas capacity | About 5,500MW of gas capacity, expected mid-to-late 2026 close | Dispatchable capacity for data-center load growth |
| PJM / Texas exposure | Capacity value rises in data-center-heavy regions | Direct beneficiary of grid shortage and capacity markets |
| Financial leverage | Q1 net debt about $19.3B | Expansion upside and financial risk coexist |
| Equity / EV scale | About $50.1B market cap and about $69.4B EV | Smaller and higher beta than CEG, but not cheap in absolute terms |

## 2. Competition And Industry Structure

VST competes in the part of the power stack where reliability, speed and geography matter. It does not have CEG's cleaner nuclear-led quality profile, and it does not have NRG's retail/VPP breadth. Instead, VST is the direct test of whether existing nuclear and gas assets can be repriced together as data centers compete for reliable power.

| Competitive vector | VST position | What would validate it | What would weaken it |
|--------------------|--------------|------------------------|----------------------|
| Nuclear contract proof | Meta agreements provide named hyperscaler evidence | Capacity, pricing, and uprate path become clearer in H2 2026 | PPA economics or regulatory path disappoint |
| Dispatchable gas | Cogentrix adds about 5,500MW | Gas capacity earns scarce-capacity economics in PJM/Texas | Gas price, emissions, or low capacity values compress upside |
| Merchant beta | Greater exposure than CEG | Capacity markets and power prices stay firm as data-center load grows | Merchant premiums roll over if AI capex expectations cool |
| Balance-sheet risk | About $19.3B net debt and active acquisition agenda | FCF and EBITDA support transactions and capital returns | Debt absorbs upside or limits flexibility |
| Peer role | Higher beta than CEG, cleaner power exposure than NRG retail | VST outperforms when power scarcity broadens beyond nuclear PPAs | VST underperforms if only clean contracted nuclear gets premium multiples |

## 3. Financial Health

VST's financial health is strong enough to make the upside interesting and levered enough to move the v5 stance to cautious rather than simply bullish. Q1 2026 revenue was $5.64B, net income was $1.03B, operating income was $1.50B, and FCF was $316M. The existing snapshot also shows TTM FCF of about $1.80B, but that cash flow must support acquisitions, capex, debt, and possible capital returns.

| Financial-health check | Current readout | Interpretation |
|------------------------|-----------------|----------------|
| Revenue base | Q1 2026 revenue of $5.64B | Smaller than CEG's post-Calpine revenue base, but meaningful scale |
| Profitability | Q1 net income of $1.03B and operating income of $1.50B | Strong quarter, helped by power-price and capacity expectations |
| Free cash flow | Q1 FCF of $316M; TTM FCF about $1.80B | Positive cash generation, but acquisition and capex needs matter |
| Leverage | About $19.3B net debt in the snapshot | Leverage amplifies both capacity upside and downside |
| Enterprise value | About $69.4B EV | EV is the right lens because debt is material |
| Transaction funding | Cogentrix framed at about $4B net purchase price and about $4.7B total consideration in public seller-side reporting | Final close mechanics should be reconciled against debt/cash/stock treatment |

## 4. Management And Caliber

Management's strategic posture is direct: own assets that can be valuable when reliability and capacity are scarce. The Meta nuclear agreements support contract credibility, while Cogentrix adds gas capacity in a market hungry for dispatchable power. The caliber test is whether management can integrate Cogentrix, keep leverage under control, and avoid turning a scarcity opportunity into a balance-sheet squeeze.

| Caliber / red-flag check | Current evidence | Report stance |
|--------------------------|------------------|---------------|
| Strategic fit | Nuclear PPAs plus Cogentrix gas capacity address baseload and dispatchable needs | Coherent for a mixed power-stack thesis |
| Contract proof | Meta 20-year agreements cover existing nuclear assets and uprates | Stronger than generic power narrative, but details still matter |
| Acquisition discipline | Cogentrix price framed around $4B net purchase price / $4.7B total consideration | Must verify final close treatment and synergy economics |
| Leverage discipline | About $19.3B net debt in Q1 snapshot | High-priority monitoring item |
| Disclosure caution | Public reports frame about 2.2GW from existing-nuclear contribution | Treat capacity figures as public-report context until company filings add more detail |
| Operating risk | Gas, capacity markets, emissions policy, and nuclear uprates all need execution | Caution is appropriate despite strong upside optionality |

## 5. Bull Case

The bull case is that AI data centers need both clean baseload and dispatchable capacity. VST owns both, and the Meta agreements validate direct hyperscaler procurement from existing nuclear assets. Cogentrix adds about 5,500MW of gas generation, which can become more valuable if PJM/Texas capacity markets continue to price data-center load growth.

VST also has higher equity beta than CEG. If CEG anchors the quality trade and NRG proves broader merchant/retail scarcity, VST can sit in the middle: more contracted than NRG, more levered and capacity-sensitive than CEG. A strong bull read requires Meta execution, Cogentrix close, firm capacity prices, and FCF that covers leverage rather than merely funding expansion.

## 6. Bear Case

The bear case is that VST is a power-scarcity trade with too much leverage and too much commodity/regulatory exposure. Q1 net debt was about $19.3B, and acquisitions plus capex can absorb cash quickly. Even with TTM FCF about $1.80B, the equity has less room for error if gas prices, capacity prices, or regulatory decisions move against the company.

The PPA details also remain important. Meta's involvement validates the category, but pricing, cost allocation, uprate timing, and regulatory approval can determine whether the economics match the market's expectations. If AI capex or data-center growth expectations cool, VST can de-rate faster than CEG because it is the higher-beta expression of the same power theme.

## 7. Key Uncertainties

VST's uncertainties are sharper than CEG's because the upside is more directly tied to capacity markets, gas assets, and leverage. The report should be re-rated only when the uncertainty table changes, not when the AI power narrative gets louder.

| Key uncertainty | Current fact base | Thesis-breaking direction |
|-----------------|-------------------|---------------------------|
| Meta PPA economics | 20-year agreements and about 2.2GW public-report frame | Capacity, pricing, cost allocation, or uprate timing disappoint |
| Cogentrix close | About 5,500MW gas capacity, expected mid-to-late 2026 close | Deal terms, integration, or debt/cash treatment reduce expected upside |
| Capacity-market pricing | PJM/Texas data-center load supports capacity value | 2026-09 PJM process or regional capacity prices fail to confirm scarcity |
| Leverage and FCF | About $19.3B net debt; Q1 FCF $316M; TTM FCF about $1.80B | Debt and capex absorb cash before shareholders receive upside |
| Gas and carbon exposure | Cogentrix increases gas capacity exposure | Emissions policy or customer low-carbon requirements cap gas asset multiples |

## 8. Valuation Context

Valuation is context, not a recommendation. Re-anchored 2026-07-31, the frame puts VST at about 22.6x TTM GAAP P/E with a $148.62 share price, on the same rough TTM GAAP EPS base near $6.57; EV/TTM sales is about 3.6x, similar to CEG despite a more merchant asset mix. The core question is not a price level, but how much the market has already paid for Meta, Cogentrix and capacity-market upside.

**Scenario grid:**

| Scenario | Driver assumptions (PPA / Cogentrix / capacity pricing / leverage) | Valuation implication (rich / fair / cheap vs today) | Probability weight |
|----------|--------------------------------------------------------------------|------------------------------------------------------|--------------------|
| Bull | Meta PPA capacity, price and uprate path becomes clearer; Cogentrix closes on time and converts about 5,500MW of gas capacity into data-center or capacity-market earnings; PJM/Texas capacity prices keep rising; FCF supports deleveraging | The current price would look fair to slightly cheap because VST gets nuclear PPA, dispatchable gas and high-beta capacity value together | 30% |
| Base | Meta PPA and Cogentrix keep advancing but the economics remain incomplete; capacity prices stay firm but volatile; TTM FCF is real but must absorb acquisitions, capex and debt; VST remains lower quality than CEG but better positioned than ordinary merchant power | The current price largely prices the base case: upside exists, but it is bought with higher balance-sheet risk | 40% |
| Bear | Cogentrix is delayed or synergies miss; PPA price, cost allocation or regulatory path disappoints; PJM/Texas capacity premiums fade; net-debt / FCF pressure rises from capex, buybacks, fuel, or regulation | The current price would look rich; high beta turns from a feature into a burden, and equity becomes more sensitive to small capacity-economics changes | 30% |

**What's priced in & the expectation gap (re-derived 2026-07-31 at the 2026-07-30 close):** At $148.62, about 22.6x TTM GAAP P/E, about 3.6x EV/TTM sales, and about $19.3B net debt, the market already prices some of the Meta nuclear PPA, Cogentrix gas capacity, and PJM/Texas capacity-scarcity story. Our base case recognizes that VST has more operating leverage than CEG, but also that net debt, gas/regulatory exposure, and capacity pricing make the bear path faster. The 30% bull / 40% base / 30% bear grid is not bearish on the assets; it shows a mildly negative expectation gap.

## 9. Catalysts And Monitoring

VST needs a stricter dashboard than CEG because the upside and downside both move faster. The next review should stay cautious unless contract, capacity, and leverage data all improve together.

| Catalyst / monitoring item | Trigger to watch | Next check | Chain implication |
|----------------------------|------------------|------------|-------------------|
| Meta PPA execution | Perry / Davis-Besse / Beaver Valley capacity, pricing, and uprate path become clearer | H2 2026 | Confirms whether existing nuclear assets can be repriced for AI load |
| Cogentrix close and integration | Deal closes on schedule, capacity contribution and synergies are clear | H2 2026 | Shows whether gas capacity adds real scarcity value |
| Capacity-market pricing | PJM / Texas data-center load lifts capacity value | 2026-09 / quarterly | Converts AI load growth into market earnings |
| Leverage and FCF | Net debt / EBITDA and FCF support acquisitions and buybacks | Quarterly | Tests whether upside survives the balance sheet |
| Gas and carbon risk | Gas price, emissions policy, or low-carbon customer requirements compress gas upside | Event-driven | Checks whether mixed power stack remains acceptable to customers |
| AI capex sensitivity | VST trades through compute-efficiency or capex-reset scares without losing the thesis | Ongoing | Separates structural power demand from sentiment beta |

## 10. Peer Comparison And Conclusion Review

VST is the high-beta comparison to CEG. It should outperform if data-center demand pushes up both clean nuclear contracts and dispatchable capacity values. It should underperform if investors decide only clean, long-term contracted power deserves the premium.

| Company | Chain role | Current valuation anchor | Main validation job |
|---------|------------|--------------------------|---------------------|
| Vistra | Cautious / nuclear-plus-gas power dashboard | $148.62 price; about $50.1B market cap; about $69.4B EV; about 22.6x TTM GAAP P/E | Test whether nuclear PPAs and gas capacity create higher-beta power upside |
| Constellation Energy | Neutral watch / power common constraint | $239.25 price; about $85.4B market cap; about $107B EV; 2026 EPS guide $11-12 | Test whether nuclear-led clean power plus Calpine dispatchable capacity earns durable hyperscaler premiums |
| NRG | Cautious / merchant-retail power dashboard | $133.97 price; about $28.3B market cap; about $52.2B EV; about 15.1x midpoint 2026 adjusted EPS | Test whether merchant gas, C&I VPP, retail power, and capacity markets monetize AI load growth |
| Read-through | Power layer as a whole | CEG/VST/NRG all reacted to PJM data-center procurement acceleration | If VST validates, the power thesis extends beyond clean nuclear; if it fails, the premium should narrow |

### Conclusion And Review

Vistra is the high-beta comparison to Constellation. CEG is the steadier nuclear-led common-constraint anchor; VST is the nuclear plus gas plus merchant-capacity dashboard. Covering it prevents the power layer from becoming only a clean nuclear story when real data centers may need a mixed power stack.

VST's chain-validation job is to test whether AI power demand spreads from clean baseload PPAs into dispatchable gas capacity, PJM/Texas capacity value, and the repricing of existing generation assets. It is higher beta than CEG and more pure-power than NRG; that lets it validate power scarcity faster, but also makes it reverse faster when gas, regulation, capacity prices, or leverage move the wrong way.

The expectation gap is mildly negative: at the $148.62 close on Jul 30, 2026, about 3.6x EV/TTM sales, about 22.6x TTM GAAP P/E, and about $19.3B net debt, the market already prices Meta nuclear PPAs, Cogentrix gas capacity, and PJM/Texas capacity scarcity; our base case recognizes real upside but does not ignore the faster downside from leverage and merchant exposure.

The current stance is **cautious, medium conviction**. The 30% bull / 40% base / 30% bear grid says the upside requires Meta PPA, Cogentrix, capacity pricing, and FCF to all deliver; the bear path only needs one or two of those variables to move against the thesis. Cautious is not a bearish call on the mixed power stack. It is the judgment that at the current price, high beta's downside speed is slightly greater than the certainty of upside.

Upgrade trigger: Move to neutral-watch or constructive if Meta PPA capacity / price / uprate path becomes clearer, Cogentrix closes on time and converts about 5,500MW of gas capacity into data-center or capacity-market earnings, PJM/Texas capacity prices keep rising, and FCF supports deleveraging. Downgrade trigger: Move to bearish-avoid if Cogentrix is delayed or synergies miss, Meta PPA economics disappoint, PJM/Texas capacity premiums fade, or net-debt / FCF pressure rises from capex, buybacks, fuel, or regulation.

## Appendix: Sources And Assumptions

- 2026Q1 revenue of $5.64B, net income of $1.03B, operating income of $1.50B, FCF of $316M, cash / debt / cash-flow tables, and the Jul 30, 2026 close of $148.62 with roughly $50.1B market cap; the cash/debt/cash-flow tables carry the caliber pulled through Yahoo Finance/yfinance on 2026-07-04.
- Meta's 20-year nuclear PPAs with Vistra, Perry / Davis-Besse / Beaver Valley uprates, and Meta's role as a major U.S. corporate nuclear buyer are sourced from Axios: https://www.axios.com/2026/01/09/meta-nuclear-deal-vistra-terrapower-oklo.
- Meta's 6GW+ nuclear framework and Vistra's roughly 2.2GW existing-nuclear contribution are sourced from Tom's Hardware: https://www.tomshardware.com/tech-industry/artificial-intelligence/meta-inks-deals-to-supply-a-staggering-6-gigawatts-in-nuclear-power-for-data-center-ambitions-enough-wattage-to-supply-5-million-homes.
- Cogentrix's transaction is framed at about a $4B net purchase price, while public seller-side reporting has also framed it near $4.7B total consideration; roughly 5,500MW gas capacity, transaction valuation, and data-center demand background are sourced from Barron's and should be reconciled against final debt / cash / stock treatment at close: https://www.barrons.com/articles/vistra-stock-price-deal-cogentrix-3000d9df.
- PJM's acceleration of data-center capacity procurement and VST / CEG / NRG market reaction are sourced from MarketWatch: https://www.marketwatch.com/story/constellations-and-vistras-stocks-rally-as-power-grid-operator-speeds-up-data-center-deals-67b95021.
